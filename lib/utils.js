...
const UA = "Mozilla/5.0 (Stremio TorrentSearch addon)";

// Lõi fetch dùng chung: timeout + retry, trả JSON đã parse
async function request(url, init, timeoutMs, retries) {
  let lastErr;
...
export function fetchJson(url, timeoutMs = 10000, retries = 1) {
  return request(url, undefined, timeoutMs, retries);
}

// POST JSON (Knaben và các API dạng POST khác)
export function postJson(url, body, timeoutMs = 10000, retries = 1) {
  return request(
    url,
    {
      method: "POST",
